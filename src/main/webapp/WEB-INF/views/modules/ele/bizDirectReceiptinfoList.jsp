...
<body>
	...
	<form:form id="searchForm" modelAttribute="bizDirectReceiptinfo" action="${ctx}/ele/bizDirectReceiptinfo/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			<li><label>用电户号：</label>
				<form:input path="scaccnum" htmlEscape="false" maxlength="15" class="input-medium"/>
			</li>
			<li><label>站点编码：</label>
			...
			<li><label>回款日期：</label>
				<input name="beginSprecdate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizDirectReceiptinfo.beginSprecdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
				<input name="endSprecdate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizDirectReceiptinfo.endSprecdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
			<li><label>票据日期：</label>
				<input name="beginScbilldate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizDirectReceiptinfo.beginScbilldate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
				<input name="endScbilldate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${bizDirectReceiptinfo.endScbilldate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bizDirectReceiptinfo">
			<tr>
				...
				<td>
					<fmt:formatDate value="${bizDirectReceiptinfo.sprecdate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${bizDirectReceiptinfo.scbilldate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${bizDirectReceiptinfo.scpredate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${bizDirectReceiptinfo.sccurdate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
